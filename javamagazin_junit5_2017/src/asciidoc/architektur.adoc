...
logische Trennung der Rollen zum Verfassen eines Tests und zur Ausführung von Tests.
Physisch spiegelt sich das in der Entkopplung der APIs zum Schreiben von denen zum Ausführen von Tests wider.
In JUnit 4 sind beide APIs in einem einzigen Artefakt enthalten.
Alle, die JUnit 4 auf irgendeine Weise verwenden,
hängen von genau diesem jar-File ab,
unabhängig davon, ob es sich dabei um IDEs, Build Tools, andere Test-Frameworks oder Testcode handelt.
Diese Verwender benutzen teilweise Interna von JUnit,
wie etwa interne Klassen oder private Instanzvariablen.
...
dass es eben kein ausreichend mächtiges API für die Integration in Tools gab.
...
image::images/architektur.png[Architektur, 400, float="left",align="left"]

Wie das Architekturdiagramm in Abb. 13 zeigt,
...
wie etwa IDEs oder Build Tools,
verwenden das Modul _junit-platform-launcher_ innerhalb der JUnit 5-Plattform,
...
Die Engine-Abstraktion ermöglicht die Koexistenz verschiedener Testframeworks bzw. -bibliotheken,
und damit insbesondere auch die Ausführung von Testfällen auf Basis von JUnit 4 und JUnit 5 innerhalb eines einzigen Testlaufs.
...
Weitere Hilfen für die Migration wie etwa der Support ausgewählter Rules finden sich im Artefakt _junit-jupiter-migrationsupport_.
Um von einem integrierenden Tool angesprochen zu werden,
müssen Test-Engines lediglich auf dem Klassenpfad vorhanden sein
und sich über den normalen _ServiceLoader_-Mechanismus der JVM [4]
beim Launcher registrieren.
...
Da eine Engine-Implementierung in vielen Fällen recht einfach zu erstellen ist [5],
sinkt somit die Schwelle,
ein sauber integriertes Test-Framework einem weiteren Benutzerkreis zur Verfügung zu stellen.
Einige Projekte haben schon gegen Milestone-Releases von JUnit 5 eigene Engines implementiert,
z.B. die jqwik-Engine für Property-Based Testing [6]
oder das BDD-Framework Specsy [7].
